...
//		int hash = 0;
		...
		//			throw new IllegalArgumentException("SendBase - getNodeIdForRecord: Operator only supports shuffling based on a SQLInteger value");
		//		};
		if (attColumn >= record.getValues().length) {
			throw new IllegalArgumentException("SendBase - getNodeIdForRecord: Specified column number to high");
		}
		...
		return (value % this.hashFunction);
	}
